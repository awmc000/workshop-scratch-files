...
public class BubbleSortArrayDiagrammer <T extends Comparable<T>> {
    ...
    public void renderSortingOperation(T[] array, String title, String filepath) { 
        //File[] fileArray = new File[array.length + 2];
        int swapNum = 1;

       // final File workingDir = new File(System.getProperty("user.dir"));
        //final File menusDir = new File(workingDir, "menus");
        //final File fastFoodMenu = new File(menusDir, "FastFoodMenu.csv");
        ...
        // Make the folder
        //String dirName = /*workingDir +*/ timeStamp + " Bubble Sort\\" ;
        ...
        for (int i = 0; i < array.length - 1; ++i) { 
            for (int j = 0; j < array.length - i - 1; ++j) {
                bufferedImage = arrayDiagrammer.draw(array, title);
        ...
        for (int i = 0; i < bufferedImages.size(); ++i) { 
            File currentReturnFile = new File(fileNames.get(i));
            try { 
                ImageIO.write(bufferedImages.get(i), "PNG", currentReturnFile);
            } catch (IOException e) { 
                System.out.println("Error writing bubble sort buffered image to file!");
                e.printStackTrace();
            }
        }
